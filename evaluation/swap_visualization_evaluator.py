...
class SwapVisualizationEvaluator(BaseEvaluator):
    ...
    def generate_mix_grid(self, model, images):
        ...
        for image in images:
            ...
            _, fea, fea1 = model(sp, gl, command="extract_feat")
            #fea = model(image, command="extract_vggfeat")
            sp = sp[:1]
            fea = fea[:1]
            fea1 = fea1[:1]
            #fea = fea[:1]
            ...
            if len(gls)==0:
                ...
                for i in range(len(gl)):
                    gls[i] = torch.cat((gls[i],gl[i]),dim=0)
        gl = gls
        ...
        grid_w = self.opt.load_size * (gl[0].size(0) + 1)
        grid_h = self.opt.load_size * (gl[0].size(0) + 1)
        ...
        for i, sp in enumerate(sps):
            sp_for_current_row = sp.repeat(gl[0].size(0), 1, 1, 1)
            fea_for_current_row = feas[i].repeat(gl[0].size(0), 1, 1, 1)
            fea1_for_current_row = feas1[i].repeat(gl[0].size(0), 1, 1, 1)
            feascat = torch.cat((torch.cat(feas,dim=0),model(torch.cat(feas1,dim=0),command="Rselfcorr")),dim=1)
            fea_for_current_row = torch.cat((fea_for_current_row,model(fea1_for_current_row,command="Rselfcorr")),dim=1)        

            corrm = model(feascat, fea_for_current_row, command="corrm")
            _, gl_w = model(torch.cat(images,dim=0), corrm, command="encode2")
            mix_row = model(sp_for_current_row, gl_w, command="decode")
            #mix_row = model(torch.cat(images,dim=0),R,command='warp')   # Show warped reference
