...
def collate_fn(batch):
    img = [b['real_A'] for b in batch]
    img = torch.stack(img, dim=0)
    mask = [b['mask_A'] for b in batch]
    mask = torch.stack(mask, dim=0)
    path = [b['path_A'] for b in batch]
    laplacian_m = [b['laplacian_m'] for b in batch]

    return {'real_A': img, 'mask_A': mask, 'path_A': path, 'laplacian_m': laplacian_m}
...
class ConfigurableDataLoader():
    ...
    def initialize(self, phase):
        ...
        if phase == "train":
            ...
            self.train_sampler = torch.utils.data.distributed.DistributedSampler(dataset, shuffle=True)
            shuffled = True
        else:
            shuffle = phase == "train" if opt.shuffle_dataset is None else opt.shuffle_dataset == "true"
            shuffled = False
        print("dataset [%s] of size %d was created. shuffled=%s" % (type(dataset).__name__, len(dataset), shuffled))
    ...
    def __next__(self):
        try:
            ...
            if self.repeat:
                self.e+=1
                self.dataloader.sampler.set_epoch(self.e)
                self.dataloader_iterator = iter(self.dataloader)
                return next(self.dataloader_iterator)
            else:
                raise StopIteration
